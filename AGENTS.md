...
## Onto-IQ course overlay (read first in class)

See [COURSE.md](./COURSE.md) for the full classroom contract.

Hard rules for this template copy:
- Learners create their own repo via **Use this template** (not Fork for class work).
- Open PRs only against the **learner repo** default branch. Never against `actualbudget/actual` unless the instructor asks.
- Paper-only finance: demo/sample data only — no real bank sync credentials.
- Prefer issues created from `.github/course-issues/` and stubs under `course/`.
- Native harness + community plugins only; cross-CLI is for independent review (Lab 07), not a custom orchestration bus.
- Command Center is optional observability and never lab pass criteria.
